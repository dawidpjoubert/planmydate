...
class MealActivityAdmin(admin.ModelAdmin):
    list_display = ('name', 'price_rating', 'is_outdoor', 'intimate', 'booking_required', 'time_of_day', 'meal_type')

@admin.register(EventActivity)
class EventActivityAdmin(admin.ModelAdmin):
    list_display = ('name', 'price_rating', 'is_outdoor', 'intimate', 'is_cardio', 'booking_required', 'time_of_day', 'event_type')
